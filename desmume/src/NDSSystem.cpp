...
#else
#define GDBSTUB_MUTEX_LOCK() \
	do                       \
	{                        \
	} while (0)
#define GDBSTUB_MUTEX_UNLOCK() \
	do                         \
	{                          \
	} while (0)
#endif
...
static int rom_init_path(const char *filename, const char *physicalName, const char *logicalFilename)
{
	...
	if (!strcasecmp(path.extension().c_str(), "zip") || !strcasecmp(path.extension().c_str(), "gz"))
	{
		type = ROM_NDS;
...
void NDS_GetCPULoadAverage(u32 &outLoadAvgARM9, u32 &outLoadAvgARM7)
{
	...
	for (size_t i = 0; i < 16; i++)
	{
		...
		u32 sample = nds.runCycleCollector[ARMCPU_ARM9][(i + 0 + nds.idleFrameCounter) & 15] + nds.runCycleCollector[ARMCPU_ARM9][(i + 1 + nds.idleFrameCounter) & 15] + nds.runCycleCollector[ARMCPU_ARM9][(i + 2 + nds.idleFrameCounter) & 15] + nds.runCycleCollector[ARMCPU_ARM9][(i + 3 + nds.idleFrameCounter) & 15];
		sample /= 4;
	...
	for (size_t i = 0; i < 16; i++)
	{
		u32 sample = nds.runCycleCollector[ARMCPU_ARM7][(i + 0 + nds.idleFrameCounter) & 15] + nds.runCycleCollector[ARMCPU_ARM7][(i + 1 + nds.idleFrameCounter) & 15] + nds.runCycleCollector[ARMCPU_ARM7][(i + 2 + nds.idleFrameCounter) & 15] + nds.runCycleCollector[ARMCPU_ARM7][(i + 3 + nds.idleFrameCounter) & 15];
		sample /= 4;
		calcLoad = calcLoad / 8 + sample * 7 / 8;
	}
