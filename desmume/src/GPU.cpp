...
void GPUEngineBase::_SpriteRenderPerform(GPUEngineCompositorInfo &compInfo, u16 *__restrict dst, u8 *__restrict dst_alpha, u8 *__restrict typeTab, u8 *__restrict prioTab)
{
	...
	for (size_t spriteNum = 0; spriteNum < 128; spriteNum++)
	{
		...
		if (spriteInfo.RotScale != 0)
		{
			...
			else if (spriteInfo.PaletteMode == PaletteMode_1x256) // If we are using 1 palette of 256 colours
			{
				...
				for (size_t j = 0; j < length; ++j, ++frameX)
				{
					...
					if (auxX >= 0 && auxY >= 0 && auxX < sprSize.width && auxY < sprSize.height)
					{
						const size_t palOffset = (MODE == SpriteRenderMode_Sprite2D) ? (auxX & 0x7) + ((auxX & 0xFFF8) << 3) + ((auxY >> 3) << 10) + ((auxY & 0x7) * 8) : (auxX & 0x7) + ((auxX & 0xFFF8) << 3) + ((auxY >> 3) * sprSize.width * 8) + ((auxY & 0x7) * 8);
			...
			else // Rotozoomed 16/16 palette
			{
				...
				for (size_t j = 0; j < length; ++j, ++frameX)
				{
					...
					if (auxX >= 0 && auxY >= 0 && auxX < sprSize.width && auxY < sprSize.height)
					{
						const size_t palOffset = (MODE == SpriteRenderMode_Sprite2D) ? ((auxX >> 1) & 0x3) + (((auxX >> 1) & 0xFFFC) << 3) + ((auxY >> 3) << 10) + ((auxY & 0x7) * 4) : ((auxX >> 1) & 0x3) + (((auxX >> 1) & 0xFFFC) << 3) + ((auxY >> 3) * sprSize.width * 4) + ((auxY & 0x7) * 4);
		...
		else // NOT rotozoomed
		{
			...
			{
				const u32 objAddress = (MODE == SpriteRenderMode_Sprite2D) ? this->_sprMem + ((spriteInfo.TileIndex) << 5) + ((spriteY >> 3) << 10) + ((spriteY & 0x7) * 8) : this->_sprMem + (spriteInfo.TileIndex << compInfo.renderState.spriteBoundary) + ((spriteY >> 3) * sprSize.width * 8) + ((spriteY & 0x7) * 8);
			...
			{
				const u32 objAddress = (MODE == SpriteRenderMode_Sprite2D) ? this->_sprMem + ((spriteInfo.TileIndex) << 5) + ((spriteY >> 3) << 10) + ((spriteY & 0x7) * 4) : this->_sprMem + (spriteInfo.TileIndex << compInfo.renderState.spriteBoundary) + ((spriteY >> 3) * sprSize.width * 4) + ((spriteY & 0x7) * 4);
...
FORCEINLINE void GPUEngineBase::_RenderLine_LayerBG_Final(GPUEngineCompositorInfo &compInfo)
{
	...
	{
	case BGType_Text:
		this->_LineText<COMPOSITORMODE, NDSColorFormat_BGR555_Rev, MOSAIC, WILLPERFORMWINDOWTEST, WILLDEFERCOMPOSITING>(compInfo);
		break;
	case BGType_Affine:
		this->_LineRot<COMPOSITORMODE, NDSColorFormat_BGR555_Rev, MOSAIC, WILLPERFORMWINDOWTEST, WILLDEFERCOMPOSITING>(compInfo);
		break;
