...
void BackupDevice::reset()
{
	...
	if (this->_state == DETECTING)
	{
		if (!memcmp(gameInfo.header.gameCode, "ASMK", 4))
			this->_addr_size = 1; // super mario 64 ds (KOR, which is different somehow)
		else if (!memcmp(gameInfo.header.gameCode, "ASM", 3))
			this->_addr_size = 2; // super mario 64 ds
		else if (!memcmp(gameInfo.header.gameCode, "BDE", 3))
			this->_addr_size = 2; // Dementium II
		else if (!memcmp(gameInfo.header.gameCode, "AL3", 3))
			this->_addr_size = 1; // spongebob atlantis squarepantis.
		else if (!memcmp(gameInfo.header.gameCode, "AH5", 3))
			this->_addr_size = 1; // over the hedge
		else if (!memcmp(gameInfo.header.gameCode, "AVH", 3))
			this->_addr_size = 1; // over the hedge - Hammy Goes Nuts!
		else if (!memcmp(gameInfo.header.gameCode, "AQ3", 3))
			this->_addr_size = 1; // spider-man 3
		else if (!memcmp(gameInfo.header.gameCode, "BPV", 3))
			this->_addr_size = 2; // puzzler world (should be eeprom 64KBits)
	...
	default:
...
#ifdef _ENABLE_MOTION
		case 0xFE:
			if (this->_motionInitState == MOTION_INIT_STATE_IDLE)
			{
				this->_motionInitState = MOTION_INIT_STATE_FE;
				return 0;
			}
			break;
		case 0xFD:
			if (this->_motionInitState == MOTION_INIT_STATE_FE)
			{
				this->_motionInitState = MOTION_INIT_STATE_FD;
				return 0;
			}
			break;
		case 0xFB:
			...
			{
				this->_motionInitState = MOTION_INIT_STATE_FB;
				return 0;
			}
			break;
		case 0xF8:
		...
		case BM_CMD_WRITEDISABLE:
			...
			case MOTION_INIT_STATE_IDLE:
				this->_motionInitState = MOTION_INIT_STATE_RECEIVED_4;
				break;
			case MOTION_INIT_STATE_RECEIVED_4:
				this->_motionInitState = MOTION_INIT_STATE_RECEIVED_4_B;
				break;
			}
#endif
			this->_write_enable = FALSE;
		...
		case BM_CMD_READHIGH:
			// printf("XHI: %08X\n",addr);
			if (this->_com == BM_CMD_WRITEHIGH)
				this->_com = BM_CMD_WRITELOW;
			if (this->_com == BM_CMD_READHIGH)
				this->_com = BM_CMD_READLOW;
			this->_addr_counter = 0;
			this->_addr = 0;
			if (this->_addr_size == 1)
			{
