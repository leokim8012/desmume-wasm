...
double samples_per_hline = (DESMUME_SAMPLE_RATE / 59.8261f) / 263.0f;
...
u8 SPU_struct::ReadByte(u32 addr)
{
	...
	case 0x509:
	{
		...
		return regs.cap[which].add | (regs.cap[which].source << 1) | (regs.cap[which].oneshot << 2) | (regs.cap[which].bits8 << 3) | (regs.cap[which].runtime.running << 7);
	}
...
u16 SPU_struct::ReadWord(u32 addr)
{
	...
	if ((addr & 0x0F00) == 0x0400)
	{
		...
		case 0x2:
			return (thischan.pan | (thischan.waveduty << 8) | (thischan.repeat << 11) | (thischan.format << 13) | ((thischan.status == CHANSTAT_PLAY) ? (1 << 15) : 0));
		case 0x8:
	...
	// SOUNDCNT
	case 0x500:
		return (regs.mastervol | (regs.ctl_left << 8) | (regs.ctl_right << 10) | (regs.ctl_ch1bypass << 12) | (regs.ctl_ch3bypass << 13) | (regs.masteren << 15));
	...
	case 0x508:
	{
		u8 val0 = regs.cap[0].add | (regs.cap[0].source << 1) | (regs.cap[0].oneshot << 2) | (regs.cap[0].bits8 << 3) | (regs.cap[0].runtime.running << 7);
		u8 val1 = regs.cap[1].add | (regs.cap[1].source << 1) | (regs.cap[1].oneshot << 2) | (regs.cap[1].bits8 << 3) | (regs.cap[1].runtime.running << 7);
...
u32 SPU_struct::ReadLong(u32 addr)
{
	...
	if ((addr & 0x0F00) == 0x0400)
	{
		...
		{
		case 0x0:
			return (thischan.vol | (thischan.volumeDiv << 8) | (thischan.hold << 15) | (thischan.pan << 16) | (thischan.waveduty << 24) | (thischan.repeat << 27) | (thischan.format << 29) | ((thischan.status == CHANSTAT_PLAY) ? (1 << 31) : 0));
		case 0x8:
...
static FORCEINLINE s32 Interpolate(const s16 *pcm16b, u8 pcm16bOffs, u32 subPos)
{
...
static FORCEINLINE void TestForLoop(SPU_struct *SPU, channel_struct *chan)
{
	...
	if (chan->sampcntInt < chan->totlength_shifted)
		return;
	...
	while (chan->sampcntInt >= chan->totlength_shifted)
		chan->sampcntInt -= step;
}

template <int CHANNELS>
FORCEINLINE static void SPU_Mix(SPU_struct *SPU, channel_struct *chan, s32 data)
{
	switch (CHANNELS)
	{
	case 0:
	...
	for (; SPU->bufpos < SPU->buflength; SPU->bufpos++)
	{
		...
		while (nSamplesToSkip--)
		{
			...
			if (chan->status != CHANSTAT_STOPPED)
			{
				...
				{
				case 0:
					data = Fetch8BitData(chan, pos);
					break;
				case 1:
					data = Fetch16BitData(chan, pos);
					break;
				case 2:
					data = FetchADPCMData(chan, pos);
					break;
				case 3:
					data = FetchPSGData(chan, pos);
					break;
				default:
					break;
				}
...
FORCEINLINE static void __SPU_ChanUpdate(const bool actuallyMix, SPU_struct *const SPU, channel_struct *const chan)
{
	...
	case 2:
		...
		break;
	case 3:
		___SPU_ChanUpdate<3, SPUInterpolation_None>(actuallyMix, SPU, chan);
		break;
	default:
		assert(false);
	}
...
FORCEINLINE static void _SPU_ChanUpdate(const bool actuallyMix, SPU_struct *const SPU, channel_struct *const chan)
{
	...
	case SPUInterpolation_Cosine:
		...
		break;
	case SPUInterpolation_CatmullRom:
		__SPU_ChanUpdate<SPUInterpolation_CatmullRom>(actuallyMix, SPU, chan);
		break;
	default:
		assert(false);
	}
...
bool spu_loadstate(EMUFILE &is, int size)
{
	...
	for (int j = 0; j < 16; j++)
	{
		...
		else if (version >= 2)
		{
			double temp;
			s64 temp2;
			is.read_doubleLE(temp);
			temp2 = (s64)(temp * (1ll << 32));
			chan.sampcntFrac = (u32)temp2;
			chan.sampcntInt = (s32)(temp2 >> 32);
			is.read_doubleLE(temp);
			temp2 = (u64)(temp * (1ull << 32)); // Intentionally unsigned
		...
		is.read_16LE(chan.x);
		if (version < 7)
			is.fseek(2, SEEK_CUR); // chan.psgnoise_last (LE16)
